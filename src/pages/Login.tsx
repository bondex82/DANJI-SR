...
export default function Login({ setUser }: { setUser: (user: any) => void }) {
  ...
  useEffect(() => {
    // Fetch APGA candidate for preloader
  ...
  if (preloading) {
    return (
      <div className="min-h-screen bg-white flex flex-col items-center justify-center p-4">
          ...
          <div className="w-48 h-48 rounded-full overflow-hidden border-4 border-brand-green shadow-2xl animate-pulse bg-slate-50 flex items-center justify-center">
            {mainCandidate?.candidate_picture ? (
              ...
              <div className="text-center p-4">
                <p className="text-brand-green font-black text-xl uppercase leading-none">{mainCandidate.name}</p>
                <p className="text-slate-400 font-bold text-[10px] mt-1 uppercase tracking-widest">{mainCandidate.party}</p>
              </div>
            ) : (
              <img 
        ...
        <p className="text-gray-500 font-medium mt-2">DANJI SS 2027 Situation Room</p>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row bg-white relative overflow-hidden">
      {/* Left Pane - Branding */}
      <div className="md:w-1/2 min-h-[40vh] md:min-h-0 bg-brand-green text-white relative overflow-hidden flex flex-col">
        {/* Background Image */}
          ...
          <div>
            <div className="flex items-center justify-between mb-12">
              ...
              <div className="w-12 h-12 bg-white rounded-lg p-1 shadow-lg">
                <img 
                  src={mainCandidate?.party_logo || "https://picsum.photos/seed/apga/100/100"} 
                  alt="Party Logo" 
                  className="w-full h-full object-contain"
                  referrerPolicy="no-referrer"
                />
            ...
            <h1 className="text-5xl md:text-7xl font-black tracking-tighter leading-[0.9] mb-6">
              DANJI SS 2027<br />
              <span className="text-brand-yellow">SITUATION ROOM</span>
            </h1>
            <div className="h-1.5 w-24 bg-brand-yellow mb-8"></div>
      ...
      <div className="md:w-1/2 flex items-center justify-center p-8 md:p-16 bg-white relative overflow-hidden">
        ...
        <div className="w-full max-w-md relative z-10">
          ...
          {error && (
            <div className="bg-red-50 border-l-4 border-brand-yellow text-brand-yellow p-4 rounded-r-xl mb-6 text-sm font-bold flex items-center gap-3 shadow-sm">
              <AlertCircle size={18} />
              {error}
            </div>
          )}
          ...
          <form onSubmit={handleLogin} className="space-y-6">
            <div>
              ...
              <input 
                ...
                placeholder="agent@profjerome2027.org"
              />
            </div>
            <div>
          ...
          <div className="mt-12 pt-8 border-t border-gray-100 flex justify-between items-center">
            <p className="text-[10px] font-bold text-gray-600 uppercase tracking-tighter">
              &copy; {new Date().getFullYear()} DANJI SS 2027
            </p>
